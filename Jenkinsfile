...
void CreateCluster(String CLUSTER_PREFIX) {
    withCredentials([string(credentialsId: 'GCP_PROJECT_ID', variable: 'GCP_PROJECT'), file(credentialsId: 'gcloud-key-file', variable: 'CLIENT_SECRET_FILE')]) {
        sh """
            ...
            gcloud container clusters create --zone us-central1-a $CLUSTER_NAME-${CLUSTER_PREFIX} --cluster-version 1.12 --machine-type n1-standard-4 --preemptible --num-nodes=3
        ...
        always {
            ...
            withCredentials([string(credentialsId: 'GCP_PROJECT_ID', variable: 'GCP_PROJECT'), file(credentialsId: 'gcloud-key-file', variable: 'CLIENT_SECRET_FILE')]) {
                sh '''
                    ...
                    gcloud auth activate-service-account --key-file $CLIENT_SECRET_FILE
                    gcloud config set project $GCP_PROJECT
                    gcloud container clusters delete --zone us-central1-a $CLUSTER_NAME-basic
                    gcloud container clusters delete --zone us-central1-a $CLUSTER_NAME-scaling
                    gcloud container clusters delete --zone us-central1-a $CLUSTER_NAME-selfhealing
                    gcloud container clusters delete --zone us-central1-a $CLUSTER_NAME-backups
                    gcloud container clusters delete --zone us-central1-a jenkins-pxc-pr-3-3cbaaae-backup
                    gcloud container clusters delete --zone us-central1-a jenkins-pxc-pr-3-1692a71-backup
                '''
            }
            sh '''
